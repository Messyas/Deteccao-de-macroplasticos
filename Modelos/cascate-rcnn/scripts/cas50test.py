...
MODEL_PATH = "/home/messyas/detectron/cascate/pesos/cascate.pth" 
OUTPUT_DIR = "./output_test_cascade" 
DATASET_TEST_NAME = "my_dataset_test_cascade" 
NUM_CLASSES = 4
IOU_THRESHOLD = 0.5  
SCORE_THRESHOLD = 0.5 
...
print("Plot da matriz confusao...")
plt.figure(figsize=(12, 10))
sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', xticklabels=cm_names, yticklabels=cm_names)
plt.xlabel('Predicted Labels')
plt.ylabel('True Labels')
plt.title(f'Confusion Matrix (Cascade - IoU={IOU_THRESHOLD}, Score={SCORE_THRESHOLD})')
cm_path = os.path.join(OUTPUT_DIR, "confusion_matrix_cascade.png")
plt.savefig(cm_path)
print(f"Matriz de Confusao salva em: {cm_path}")
plt.close() 

print("Salvando metricas em CSV...")
csv_path = os.path.join(OUTPUT_DIR, "evaluation_metrics_cascade.csv") 
with open(csv_path, 'w', newline='') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(['Metric', 'Value'])
    writer.writerow(['mAP (COCO 50:95)', map_coco])
    writer.writerow(['mAP50 (COCO)', map50_coco])
    writer.writerow(['mAP75 (COCO)', map75_coco])
    writer.writerow(['---', '---'])
...
visualize_count = 5
output_viz_dir = os.path.join(OUTPUT_DIR, "visualizations_cascade") 
...
for i, d in enumerate(random.sample(dataset_dicts, visualize_count)):
    ...
    img_path = os.path.join(output_viz_dir, f"prediction_cascade_{i}.jpg") 
    cv2.imwrite(img_path, out.get_image()[:, :, ::-1])
    print(f"Visualização salva em: {img_path}")

print("\nScript de teste (Cascade RCNN) concluído!")
